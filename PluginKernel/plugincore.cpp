...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
	...
	double finalVolumeLeft = volume;
	double finalVolumeRight = volume;

	if (compareEnumToInt(channelsEnum::left, channels)) 
		finalVolumeRight = 0.0;
	else if (compareEnumToInt(channelsEnum::right, channels))
		finalVolumeLeft = 0.0;
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFMono)
    {
        ...
        processFrameInfo.audioOutputFrame[0] = finalVolumeLeft * processFrameInfo.audioInputFrame[0];
		leftOutMeter = processFrameInfo.audioOutputFrame[0];
		rightOutMeter = processFrameInfo.audioOutputFrame[0];
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFStereo)
    {
		// --- pass through code: change this with your signal processing
        processFrameInfo.audioOutputFrame[0] = finalVolumeLeft * processFrameInfo.audioInputFrame[0];
        processFrameInfo.audioOutputFrame[1] = finalVolumeRight * processFrameInfo.audioInputFrame[0];

		leftOutMeter = processFrameInfo.audioOutputFrame[0];
		rightOutMeter = processFrameInfo.audioOutputFrame[1];
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFStereo)
    {
		// --- pass through code: change this with your signal processing
        processFrameInfo.audioOutputFrame[0] = finalVolumeLeft * processFrameInfo.audioInputFrame[0];
        processFrameInfo.audioOutputFrame[1] = finalVolumeRight * processFrameInfo.audioInputFrame[1];
...
bool PluginCore::guiParameterChanged(int32_t controlID, double actualValue)
{
	...
	switch (controlID){
		case controlID::presetSelection:{
			// --- setup a host message for the GUI
			HostMessageInfo hostMessageInfo;
			hostMessageInfo.hostMessage = sendGUIUpdate;

			// --- manipulate the volume parameter
			GUIParameter volParam;
			volParam.controlID = controlID::volume;

			// --- NOTE: compare with double value
			if (actualValue == 0.0) // full
				volParam.actualValue = 1.0;
			else if (actualValue == 1.0) // medium
				volParam.actualValue = 0.5;
			else if (actualValue == 2.0) // quite
				volParam.actualValue = 0.25;

			// --- add to the list; add as many more as you like
			hostMessageInfo.guiUpdateData.guiParameters.push_back(volParam);
...
bool PluginCore::initPluginParameters()
{
	...
	piParam = new PluginParameter(controlID::volume, "Volume", "", controlVariableType::kDouble, 0.000000, 1.000000, 0.707000, taper::kAntiLogTaper);
	...
	// --- meter control: LOUT
	piParam = new PluginParameter(controlID::leftOutMeter, "LOUT", 10.00, 100.00, ENVELOPE_DETECT_MODE_RMS, meterCal::kLogMeter);
	piParam->setBoundVariable(&leftOutMeter, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- meter control: ROUT
	piParam = new PluginParameter(controlID::rightOutMeter, "ROUT", 10.00, 100.00, ENVELOPE_DETECT_MODE_RMS, meterCal::kLogMeter);
	piParam->setBoundVariable(&rightOutMeter, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- discrete control: Channels
	piParam = new PluginParameter(controlID::channels, "Channels", "stereo,left,right", "stereo");
	piParam->setBoundVariable(&channels, boundVariableType::kInt);
	piParam->setIsDiscreteSwitch(true);
	addPluginParameter(piParam);

	// --- discrete control: Preset Selection
	piParam = new PluginParameter(controlID::presetSelection, "Preset Selection", "full,medium,quiet", "full");
	piParam->setBoundVariable(&presetSelection, boundVariableType::kInt);
	piParam->setIsDiscreteSwitch(true);
	addPluginParameter(piParam);
	...
	// --- controlID::leftOutMeter
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(201326592);
	setParamAuxAttribute(controlID::leftOutMeter, auxAttribute);

	// --- controlID::rightOutMeter
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(201326592);
	setParamAuxAttribute(controlID::rightOutMeter, auxAttribute);
	...
	// --- controlID::presetSelection
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(805306368);
	setParamAuxAttribute(controlID::presetSelection, auxAttribute);
...
bool PluginCore::initPluginPresets()
{
	...
	setPresetParameter(preset->presetParameters, controlID::volume, 0.707000);
	...
	setPresetParameter(preset->presetParameters, controlID::presetSelection, -0.000000);
	addPreset(preset);
